...
__copyright__ = "Copyright (C) 2022 Leonardo Pinheiro"
...
# class DatabaseObject(object):     # Python 2.7.11
class DatabaseObject(ABC):          # Python 3.8.10 (debugged)
    """Abstract superclass to be instantiated from all the others that access
    the database.
    ...
    # __metaclass__ = ABCMeta     # Python 2.7.11

    # ----- START OF ACTIVE RECORD CODE -----

    _database = ConnectionDB()
    """activerecord.connection_db.ConnectionDB: Holds the connection
    ...
    def find_by_sql(cls, sql):
        """Sends the SQL query to the database and returns a list of objects.
    ...
    @abstractmethod
    def _validate(self):
        """Every class that extends this one (DatabaseObject) must implement
        this method.
    ...
    def _create(self):
        ...
        for key in attributes:
            ...
            # if key == attributes.keys()[-1]:      # Python 2.7.11
            if key == list(attributes.keys())[-1]:  # Python 3.8.10 (debugged)
                break
    ...
    def save(self):
        """Executes the _update() or the _create() instance methods based on the
        presence of an ID.
    ...
    def merge_attributes(self, **kwargs):
        """Merges the attributes from the given dictionary into the object in
    ...
    def attributes(self):
        """Creates a dictionary that have, as properties, the database columns
    ...
    def _sanitized_attributes(self):
        """Sanitizes (escapes the values) of the object before sending to the
        database.
