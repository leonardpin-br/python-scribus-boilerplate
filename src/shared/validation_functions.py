"""Usefull validation functions available to other packages in this project.
...
def has_presence(value):
    """Validate data presence. Reverse of isBlank().
...
def has_length_greater_than(value, min):
    """Validate string length. Spaces count towards length. Use ``trim()`` if
...
def has_length(value, options):
    """Validate string length. Combines functions greater_than, less_than and
